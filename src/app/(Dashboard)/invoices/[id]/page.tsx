...
export default function InvoiceDetailPage() {
    ...
    try {
      ...
      const formValues: InvoiceFormValues = {
        ...
        issue_date: formData.issueDate
          ? new Date(formData.issueDate.toString().split('T')[0] + 'T00:00:00')
          : new Date(new Date().toLocaleDateString()),
        due_date: formData.dueDate
          ? new Date(formData.dueDate.toString().split('T')[0] + 'T00:00:00')
          : new Date(new Date().toLocaleDateString()),
        service_type:
          (formData.serviceType as unknown as string)?.toLowerCase() ===
          'retainers'
            ? 'Retainers'
            : (formData.serviceType as unknown as string)?.toLowerCase() ===
                'app development'
        ...
        status:
          (formData.status as unknown as string)?.toLowerCase() === 'draft'
            ? 'Draft'
            : (formData.status as unknown as string)?.toLowerCase() === 'sent'
              ? 'Sent'
              : (formData.status as unknown as string)?.toLowerCase() === 'paid'
                ? 'Paid'
                : (formData.status as unknown as string)?.toLowerCase() ===
                    'overdue'
                  ? 'Overdue'
                  : (formData.status as unknown as string)?.toLowerCase() ===
                      'cancelled'
      ...
      const invoiceResponse: Invoice = {
        ...
        serviceType:
          (updatedInvoice.serviceType?.toLowerCase() as ServiceType) ||
          'web development',
        description: updatedInvoice.description,
        amount:
          typeof updatedInvoice.amount === 'number'
            ? updatedInvoice.amount
            : typeof updatedInvoice.amount === 'string'
              ? parseFloat(updatedInvoice.amount) || 0
              : 0,
        currency: (updatedInvoice.currency as Currency) || 'USD',
        status:
          (updatedInvoice.status?.toLowerCase() as InvoiceStatus) || 'draft',
  ...
  function handleDownloadPDF() {
    ...
    try {
      ...
      doc.setFontSize(12);
      doc.text(`No: ${invoice.invoiceNumber}`, pageWidth - 15, 20, {
        align: 'right',
      });
      ...
      if (invoice.companyName) {
        doc.text(`${invoice.companyName}`, 15, y);
        y += 6;
      }
      ...
      doc.text(`${invoice.amount.toLocaleString()}`, pageWidth - 15, y, {
        align: 'right',
      });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {invoice.companyName || 'Individual'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="col-span-4 space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="text-center">
              <Badge
                variant="outline"
                className={`${getStatusColor(invoice.status)} text-lg px-4 py-2 capitalize`}
              >
                {invoice.status}
              </Badge>
            </div>
