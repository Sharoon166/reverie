...
export default function ClientDetailClient({
  ...
  async function handleExportCard() {
    try {
      toast.loading(`Generating client card for ${client.name}...`, {
        id: 'client-card-download',
      });
      const doc = new jsPDF();
      ...
      doc.text(
        `Revenue: ${client.totalSpent} ${client.totalSpentCurrency}`,
        30,
        130
      );
      ...
      doc.save(fileName);
      toast.success(`Client card for ${client.name} downloaded successfully`, {
        id: 'client-card-download',
      });
    } catch (error) {
      console.error('Card generation error:', error);
      toast.error('Failed to generate client card. Please try again.', {
        id: 'client-card-download',
      });
    }
  }
  ...
  async function handleDownloadPDF() {
    try {
      toast.loading(`Generating PDF for ${client.name}...`, {
        id: 'client-pdf-download',
      });
      const doc = new jsPDF();
      const pageWidth = doc.internal.pageSize.width;
      ...
      const statusColor: [number, number, number] =
        client.status === 'Active' ? [34, 197, 94] : [107, 114, 128];
      ...
      doc.text(`Client ID: ${client.id}`, pageWidth - 20, footerY, {
        align: 'right',
      });
      const fileName = `client-${client.name.replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(fileName);
      toast.success(
        `Client report for ${client.name} downloaded successfully`,
        { id: 'client-pdf-download' }
      );
    } catch (error) {
      console.error('PDF generation error:', error);
      toast.error('Failed to generate client PDF. Please try again.', {
        id: 'client-pdf-download',
      });
    }
  }
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <Badge
                    variant="outline"
                    className={getSourceColor(client.source)}
                  >
                    {client.source}
                  </Badge>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {client.numberOfProjects || 0} completed
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-green-700">
                  {client.totalProfit
                    ? formatPakistaniCurrency(client.totalProfit)
                    : 'N/A'}
                </p>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-purple-700">
                  {client.retainer
                    ? formatPakistaniCurrency(client.retainer)
                    : 'N/A'}
                </p>
              </div>
            </div>
        ...
        <div className="col-span-4 space-y-6">
          <Card className="p-6">
            ...
            <div className="text-center">
              <Badge
                variant="outline"
                className={`${getStatusColor(client.status)} text-lg px-4 py-2`}
              >
                {client.status}
              </Badge>
            </div>
          ...
          <form className="space-y-4" onSubmit={handleSave}>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <select
                ...
                value={formData.totalSpentCurrency ?? ''}
                onChange={(e) =>
                  setFormData((f) => ({
                    ...f,
                    totalSpentCurrency: e.target.value as Currency,
                  }))
                }
                disabled={isLoading}
              >
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <select
                className="border rounded px-3 py-2"
                value={formData.totalProfitCurrency ?? ''}
                onChange={(e) =>
                  setFormData((f) => ({
                    ...f,
                    totalProfitCurrency: e.target.value as Currency,
                  }))
                }
                disabled={isLoading}
              >
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData((f) => ({
                    ...f,
                    retainer: Number(e.target.value),
                  }))
                }
                disabled={isLoading}
              />
