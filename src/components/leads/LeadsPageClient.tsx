...
export default function LeadsPageClient({
  ...
  const getQuarter = useCallback(
    (d: Date) => Math.floor(d.getMonth() / 3) + 1,
    []
  );
  const sameQuarter = useCallback(
    (dateStr: string, ref: Date) => {
      const d = new Date(dateStr);
      return (
        d.getFullYear() === ref.getFullYear() &&
        getQuarter(d) === getQuarter(ref)
      );
    },
    [getQuarter]
  );

  const leadsThisQuarter = leads.filter((l) =>
    sameQuarter(String(l.$updatedAt), now)
  )

  const totalQualifiedThisQuarter = useMemo(
    () =>
      leadsThisQuarter.filter(
        (l) => String(l.status).toLowerCase() === 'qualified'
      ).length,
    [leadsThisQuarter]
  );
  const convertedThisQuarter = useMemo(
    () =>
      leadsThisQuarter.filter(
        (l) => String(l.status).toLowerCase() === 'converted'
      ).length,
    [leadsThisQuarter]
  );
  const conversionRate = useMemo(
    () =>
      totalQualifiedThisQuarter > 0
        ? Math.round((convertedThisQuarter / totalQualifiedThisQuarter) * 100)
        : 0,
    [convertedThisQuarter, totalQualifiedThisQuarter]
  );

  const filteredLeads = leadsThisQuarter
  // .filter(
  //   (lead) =>
  ...
  const pagination = usePagination({ data: leads, itemsPerPage: 6 });
  ...
  // Quarterly KPIs
  const proposalsSent = leadsThisQuarter.filter(
    (l) => String(l.status).toLowerCase() === 'proposal'
  ).length;
  const meetingsBooked = leadsThisQuarter.filter(
    (l) => String(l.status).toLowerCase() === 'meeting'
  ).length;
  ...
  function valuesToFormData(values: LeadFormValues) {
    ...
    if (values.estimatedValue !== undefined)
      fd.set('estimatedValue', String(values.estimatedValue));
    if (values.currency) fd.set('currency', values.currency);
    if (values.nextFollowup)
      fd.set('nextFollowup', values?.nextFollowup.toISOString().split('T')[0]);
  ...
  function handleDownloadPDF() {
    try {
      ...
      let y = 35;
      doc.text(
        `Current Quarter: Q${getQuarter(now)} ${now.getFullYear()}`,
        15,
        y
      );
      y += 6;
      doc.text(`Total Leads: ${filteredLeads.length}`, 15, y);
      y += 6;
      doc.text(`Conversion Rate: ${conversionRate}%`, 15, y);
      y += 10;
      doc.setFont('helvetica', 'bold');
      doc.text('Leads', 15, y);
      y += 6;
      ...
      filteredLeads.forEach((l, idx) => {
        ...
        doc.text(
          `${idx + 1}. ${l.name} (${l.status}) - ${l.company || 'Individual'}`,
          15,
          y
        );
        y += 6;
    ...
    try {
      if (formMode === 'create') {
        ...
        const created = {
          ...(createdRaw as Lead),
          id: String(
            (createdRaw as Lead)?.id ?? (createdRaw as Lead)?.$id ?? ''
          ),
        } as Lead;
        ...
        const updated = {
          ...(updatedRaw as Lead),
          id: String(
            (updatedRaw as Lead)?.id ?? (updatedRaw as Lead)?.$id ?? ''
          ),
        } as Lead;
      ...
      toast.error(
        `Failed to ${formMode === 'create' ? 'create' : 'update'} lead. Please try again.`
      );
    }
  }
    ...
    startTransition(async () => {
      try {
        const createdListRaw = await Promise.all(
          data.map(async (row) => {
            ...
            map(
              'estimatedValue',
              Number(
                (row as Lead).estimatedValue ?? (row as Lead).estimatedValue
              ) || 0
            );
        ...
        const createdList = createdListRaw.map(
          (r) => ({ ...(r as Lead), id: String((r as Lead)?.id ?? '') }) as Lead
        );
    ...
    () => [
      {
        ...
        description: 'Qualified to Converted',
      ...
      },
      {
        ...
        value: `${proposalsSent}`,
        description: `Target: ${quarter?.proposalsSentTarget}`,
        icon: <Mail className="h-5 w-5" />,
        color: 'bg-green-100 text-green-800',
        progress: quarter?.proposalsSentTarget ? (proposalsSent / quarter?.proposalsSentTarget) * 100 : 0,
        visible: true,
      },
      {
        title: 'Meetings Booked',
        value: `${meetingsBooked}`,
        description: `Target: ${quarter?.meetingsBookedTarget}`,
        icon: <Calendar className="h-5 w-5" />,
        color: 'bg-purple-100 text-purple-800',
        progress: quarter?.meetingsBookedTarget ? (meetingsBooked / quarter?.meetingsBookedTarget) * 100 : 0,
        visible: true,
      },
      {
        title: 'Agency Partnerships',
        value: `${agencyPartnerships}`,
        description: `Target: ${TARGETS.agencyPartnerships}`,
        icon: <Users className="h-5 w-5" />,
        color: 'bg-yellow-100 text-yellow-800',
        progress: (agencyPartnerships / TARGETS.agencyPartnerships) * 100,
      },
    ],
    [
      conversionRate,
  ...
  return (
    <div className="pt-16 space-y-4">
        ...
        <div>
          <Card className="p-6 border-0 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-3xl font-light text-gray-900">
                  {formatPakistaniCurrency(totalValue)}
                </div>
      ...
      <div className="w-full mt-6">
        <h2 className="text-2xl font-semibold mb-4">Quarterly KPIs  <span className="uppercase">{quarter?.quarterId}</span></h2>
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-yellow-500 rounded-xl flex items-center justify-center text-black font-bold text-lg">
                      {lead.name.charAt(0)}
                    </div>
                    <div>
                ...
                <div className="flex justify-between items-center gap-4 pt-4 border-t">
                  <div className="text-center">
                    <div className="text-xl font-semibold">
                      {lead.estimatedValue
                        ? `${formatPakistaniCurrency(lead.estimatedValue)}`
                        : 'N/A'}
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-gray-500">
                    {lead.assignedTo
                      ? `Assigned: ${getEmployeeName(lead.assignedTo)}`
                      : 'Unassigned'}
                  </div>
                  <DropdownMenu>
      ...
      {/* Quarter Targets Dialog */}
      <LeadQuarterTargetsDialog
        quarter={quarter}
