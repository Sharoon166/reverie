...
export default function InvoicesPageClient({
  ...
  // Separate and sort invoices
  const { retainerInvoices, normalInvoices } = useMemo(() => {
    const retainer: InvoiceRow[] = [];
    const normal: InvoiceRow[] = [];

    filteredInvoices.forEach((invoice) => {
      if (invoice.serviceType === 'Retainers') {
        retainer.push(invoice);
      } else {
        normal.push(invoice);
      }
    });

    const sortInvoices = (arr: InvoiceRow[]) => {
      return [...arr].sort((a, b) => {
        const rank = (inv: InvoiceRow) =>
          inv.status === 'Paid' ? 2 : isOverdue(inv) ? 0 : 1;
        const rA = rank(a),
          rB = rank(b);
    ...
    return {
      retainerInvoices: sortInvoices(retainer),
      normalInvoices: sortInvoices(normal),
    };
  }, [filteredInvoices]);

  // Separate pagination for each type
  const retainerPagination = usePagination({
    data: retainerInvoices,
    itemsPerPage: 6,
  });
  const normalPagination = usePagination({
    data: normalInvoices,
    itemsPerPage: 6,
  });
  ...
  async function handleMarkPaid(invoice: InvoiceRow) {
    try {
      ...
      setInvoices((prev) =>
        prev.map((inv) =>
          inv.$id === invoice.$id
            ? { ...inv, status: updated.status, paidDate: updated.paidDate }
            : inv
        )
      );
  ...
  async function handleFormSubmit(data: InvoiceFormValues) {
    try {
      ...
      toast.error(
        `Failed to ${formMode === 'create' ? 'create' : 'update'} invoice. Please try again.`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const exportInvoicePDF = (invoice: InvoiceRow) => {
    try {
      ...
      doc.setFontSize(10);
      doc.text(`Invoice #: ${invoice.invoiceNumber}`, pageWidth - margin, 20, {
        align: 'right',
      });
      doc.text(
