...
function parseLeadForm(formData: FormData): LeadPayload {
  ...
  // Numeric and enum
  if (get('estimatedValue'))
    payload.estimatedValue = toNum(get('estimatedValue'));
...
export async function getLeadsById(id: string): Promise<modifiedLead> {
  const [leadRes, employeesRes] = await Promise.all([
    db.listRows({
      databaseId: APPWRITE_DB.databaseId,
      tableId: APPWRITE_DB.tables.leads,
  ...
  const lead = leadRes.rows[0] as unknown as modifiedLead;

  if (lead.assignedTo) {
    const employee = employeesRes.rows.find(
      (emp: any) => emp.$id === lead.assignedTo
    );
    if (employee) {
      return {
        ...lead,
        assignedTo: employee.name || lead.assignedTo,
      };
    }
  }
