...
export async function getDashboardStats(): Promise<DashboardStats> {
  try {
    ...
    console.log(`Calculating Cash on Hand for Q${quarter}-${year} (${startDate} to ${endDate})`);
    ...
    try {
      ...
      console.log(`Found ${allInvoices.rows?.length || 0} total invoices, ${paidInvoices.rows.length} paid`);
      ...
      const salaryPayments = {
        rows: (allSalaryPayments.rows || []).filter(payment => {
          ...
          return quarterMonths.includes(month) && (status === 'paid' || hasPaidDate);
        })
      };

      console.log(`Found ${allSalaryPayments.rows?.length || 0} total salary payments, ${salaryPayments.rows.length} for current quarter`);
      ...
      console.log(`Found ${allClients.rows?.length || 0} total clients, ${clients.rows.length} active`);

      // Calculate revenue from PAID invoices in current quarter
      const quarterlyRevenue = (paidInvoices.rows || []).reduce((sum, invoice) => {
      ...
      const totalExpenses = (quarterExpenses.rows || []).reduce((sum, expense) => {
      ...
      const totalSalaries = (salaryPayments.rows || []).reduce((sum, payment) => {
        const amount = parseFloat(String(payment.netAmount || payment.amount)) || 0;
        if (amount < 0) {
      ...
      const cashOnHand = Math.max(0, quarterlyRevenue - (totalExpenses + totalSalaries));

      // Calculate profit and profit margin
      const profit = quarterlyRevenue - totalExpenses - totalSalaries;
      const profitMargin = quarterlyRevenue > 0 ? (profit / quarterlyRevenue) * 100 : 0;
    ...
    return [
      {
        ...
        changeType: 'increase',
        icon: 'DollarSign',
        progress: Math.min(100, (stats.quarterlyRevenue / (stats.quarterlyRevenue * 1.1)) * 100),
        color: 'bg-green-500',
      ...
      },
      {
        ...
        changeType: 'increase',
        icon: 'TrendingUp',
        progress: Math.min(100, (stats.profitMargin / (stats.profitMargin * 1.05)) * 100),
        color: 'bg-blue-500',
      ...
      },
      {
        ...
        changeType: 'increase',
        icon: 'DollarSign',
        progress: Math.min(100, (stats.cashOnHand / (stats.cashOnHand * 1.15)) * 100),
        color: 'bg-green-500',
      ...
      },
      {
        ...
        changeType: 'increase',
        icon: 'Users',
        progress: Math.min(100, (stats.activeClients / (stats.activeClients + 5)) * 100),
        color: 'bg-blue-500',
      ...
      },
      {
        ...
        changeType: 'decrease',
        icon: 'CheckCircle2',
        progress: Math.min(100, ((stats.pendingTasks - 10) / stats.pendingTasks) * 100 || 0),
        color: 'bg-yellow-500',
      ...
      },
      {
        ...
        color: stats.teamPerformance >= 90 ? 'bg-green-500' : stats.teamPerformance >= 70 ? 'bg-yellow-500' : 'bg-red-500',
